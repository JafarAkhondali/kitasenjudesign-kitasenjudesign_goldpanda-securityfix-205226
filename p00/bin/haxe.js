...
Main3d.prototype = {
	...
	,setBgColor: function(isBlue) {
		if(isBlue) this.renderer.setClearColor(new THREE.Color(16777215)); else this.renderer.setClearColor(new THREE.Color(0));
	}
	,_onInit: function() {
...
faces.MaeFace.prototype = $extend(THREE.Object3D.prototype,{
	...
	,setTexture: function(nn) {
		this._face.getMat().setTexture(nn);
	}
});
...
faces.MaeFaces.prototype = $extend(THREE.Object3D.prototype,{
	...
	,_keyDownFunc: function(e) {
		...
		case 66:
			this._setBlue();
			this._main.setBgColor(this._isBlue);
			break;
		}
	}
	,_setBlue: function() {
		...
		while(_g1 < _g) {
			var i = _g1++;
			this._faces[i].setTexture(this._isBlue?1:0);
		}
	}
...
faces.MaeShaderMaterial = function() {
	...
	if(faces.MaeShaderMaterial._texture2 == null) faces.MaeShaderMaterial._texture2 = THREE.ImageUtils.loadTexture("../../assets/" + "face/dede_face_diff_blue.png");
	this._indecies = [];
	this._freq = [];
	var _g1 = 0;
...
faces.MaeShaderMaterial.prototype = $extend(THREE.ShaderMaterial.prototype,{
	...
	,setTexture: function(index) {
		if(index == 0) this.uniforms.texture.value = faces.MaeShaderMaterial._texture1; else this.uniforms.texture.value = faces.MaeShaderMaterial._texture2;
	}
	,setWireframe: function(b) {
		if(b) {
...
faces.data.MaeFormV = function() {
	this._cams = [new faces.data.CamData(215,0,0),new faces.data.CamData(215,0,-0.5),new faces.data.CamData(215,0,0.5),new faces.data.CamData(312,0,0),new faces.data.CamData(116,-0.34,-0.59),new faces.data.CamData(90,0,0),new faces.data.CamData(209,0.34,-0.59)];
	faces.data.MaeFormBase.call(this);
};
...
faces.data.MaeFormVpers = function() {
	this._cams = [new faces.data.CamData(255,0,0),new faces.data.CamData(255,0,0),new faces.data.CamData(255,0,0),new faces.data.CamData(255,0,0),new faces.data.CamData(255,0,0),new faces.data.CamData(255,-0.73,-0.1),new faces.data.CamData(255,0,0),new faces.data.CamData(255,0,0),new faces.data.CamData(255,0.63,-0.1)];
	faces.data.MaeFormBase.call(this);
};
