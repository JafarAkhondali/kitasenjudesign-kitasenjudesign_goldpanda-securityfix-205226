...
class Webcam
{
	 ...
	 * @param	callback
	 */
	public function init(sizeW:Int,sizeH:Int,callback:Void->Void):Void {
		
		_callback = callback;
		video = Browser.document.createVideoElement();
		//getElementById("vi");	
		video.width = cast sizeW;
		video.height = cast sizeH;
	...
	function _onErr(err) 
	{
		...
		//Browser.document.getElementById("info").innerHTML = "Your webcam is not be detected.";
		//Browser.document.getElementById("info").style.display = "none";
		//img = Browser.document.createImageElement();
		//img.onload = cast _onLoadImg;
		//img.src = "mona.jpg";
	}
	...
	function _onHoge(stream:LocalMediaStream):Bool 
	{
		...
		//trace("size =" + video.width + " " + video.height);
