...
BeyondCodeGeo.getFillMesh = function(nn,font) {
	if(BeyondCodeGeo.mat == null) {
		...
		BeyondCodeGeo.mat.side = 2;
	}
	Tracer.debug(">>>>>>" + nn);
	var o = new THREE.Object3D();
	var name = font + "_" + nn + "2";
	var geo = BeyondCodeGeo._mojiGeo.get(name);
	if(geo == null) {
		if(font == 1) geo = typo.FontMeshMaker.getGeometry(nn,font); else geo = BeyondCodeGeo.getFillGeo(nn,font);
		BeyondCodeGeo._mojiGeo.set(name,geo);
	}
	if(geo != null) {
		var line = new THREE.Mesh(geo,BeyondCodeGeo.mat);
...
BeyondCodeGeo._getGeo = function(nn,font) {
	var geos = [];
	var strokes = typo.StrokeUtil.getStrokes(nn,1.3,font);
	var _g1 = 0;
	...
	while(_g1 < _g) {
		...
		while(_g3 < _g2) {
			var i = _g3++;
			g.vertices.push(new THREE.Vector3(vv[i].x,-vv[i].y,0));
		}
		g.vertices.push(new THREE.Vector3(vv[0].x,-vv[0].y,0));
		geos.push(g);
	}
	return geos;
};
BeyondCodeGeo._getShape = function(nn,font) {
	var geos = [];
	var strokes = typo.StrokeUtil.getStrokes(nn,1.3,font);
	var _g1 = 0;
	var _g = strokes.length;
	while(_g1 < _g) {
		...
		while(_g3 < _g2) {
			var i = _g3++;
			if(i == 0) g.moveTo(vv[i].x,-vv[i].y); else g.lineTo(vv[i].x,-vv[i].y);
		}
		g.lineTo(vv[0].x,-vv[0].y);
		geos.push(g);
	}
	return geos;
};
...
clock.DotDigit.prototype = $extend(THREE.Object3D.prototype,{
	...
	,setStrokes: function(str,scale,space,font) {
		...
		if(this._moji != str || font != this._font) {
			...
			this._outline = BeyondCodeGeo.getLine(str,font);
...
dede.DeDeDigit.prototype = $extend(THREE.Object3D.prototype,{
	...
	,setStrokes: function(str,scale,space,font) {
		...
		this.outline = BeyondCodeGeo.getFillMesh(str,this._font);
...
dede.DeDeLine.prototype = $extend(THREE.Object3D.prototype,{
	...
	,reset: function(type,data,isTypeRandom) {
		...
		while(_g1 < _g) {
			...
			this._digits[i].position.y = this._data.offsetY;
...
dede.cuts.DeDeCutMultiLine.prototype = $extend(dede.cuts.DeDeCutBase.prototype,{
	...
	,_reposOne: function() {
		...
		this._lines.setGeoMax(150,[true,false,false,false]);
	...
	,_reposTwo: function() {
		this._lines.visible = true;
		this._lines.setGeoMax(150,[true,true,false,false]);
		this._lines.reposition(2,150,-75);
		this._vrdg.visible = false;
		this._vrdg.setGeoMax(1);
		this._cam.setZoom(3.6);
	}
...
dede.cuts.DeDeCutMultiLineY = function() {
	this._lineType = 0;
	dede.cuts.DeDeCutBase.call(this);
};
dede.cuts.DeDeCutMultiLineY.__super__ = dede.cuts.DeDeCutBase;
dede.cuts.DeDeCutMultiLineY.prototype = $extend(dede.cuts.DeDeCutBase.prototype,{
	init: function(main) {
		this._cam = main._camera;
		this._lines = main._lines;
		this._vrdg = main._vrdg;
...
dede.cuts.DeDeCutOneLine.prototype = $extend(dede.cuts.DeDeCutBase.prototype,{
	start: function() {
		...
		this.data.txt = "DEDEMOUSE @RINGOFES ";
		this.data.font = 1;
...
typo.FontMeshMaker = function() {
};
typo.FontMeshMaker.getGeometry = function(moji,f) {
	if(moji == " ") return null;
	var key = f + "_" + moji;
	if(typo.FontMeshMaker.map.get(key) != null) return typo.FontMeshMaker.map.get(key);
	var ff = typo.StrokeUtil.getFont(f);
	var shape = new THREE.Shape();
	typo.FontMeshMaker.getShape(shape,moji,true,1.3,ff);
	var geo = new THREE.ShapeGeometry(shape,{ });
	typo.FontMeshMaker.map.set(key,geo);
	return geo;
};
typo.FontMeshMaker.getShape = function(g,moji,isCentering,scale,letter) {
	if(scale == null) scale = 1;
	if(isCentering == null) isCentering = false;
	var shape = g;
	var motif = letter.motifs.get(moji);
...
typo.StrokeUtil.getFont = function(fontIndex) {
	return typo.StrokeUtil._fonts[fontIndex];
};
...
BeyondCodeGeo._mojiGeo = new haxe.ds.StringMap();
...
dede.cuts.DeDeString.texts = [{ text : "デデデデデ", font : 0, spaceX : 30, offsetY : 20},{ text : "RINGO", font : 1, spaceX : 40, offsetY : 0},{ text : "DEDEMOUSE", font : 1, spaceX : 40, offsetY : 0},{ text : "デデデデデ", font : 0, spaceX : 30, offsetY : 20},{ text : "DEDE", font : 1, spaceX : 40, offsetY : 0},{ text : "KITASENJUDESIGN", font : 1, spaceX : 40, offsetY : 0},{ text : "デデデデデ", font : 0, spaceX : 30, offsetY : 20},{ text : "TOKYODESIGNWEEK2016", font : 1, spaceX : 40, offsetY : 0}];
...
typo.FontMeshMaker.map = new haxe.ds.StringMap();
