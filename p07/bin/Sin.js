...
MainDrawer.prototype = $extend(createjs.Container.prototype,{
	init: function(data,moji,callback) {
		this._moji = moji;
		this._data = data;
	...
	,_makeTypo0: function() {
		...
		var ox = -width / 2;
		var scale = window.innerWidth / width * 0.4;
		if(str.length >= 3) scale = window.innerWidth / width;
...
Maps.prototype = {
	...
	,_start: function() {
		...
		this._map2 = new ModuleMap(1);
		this._map1.init(this._loader);
		this._map2.init(this._loader);
	...
	,_tween: function() {
		console.log("_tween");
		this._map2.tween(40000);
		this._map2.startRot(12000);
		this._map2.next(this._getMoji());
		haxe.Timer.delay($bind(this,this._tween2),20000);
	}
	,_tween2: function() {
		console.log("_tween2");
		this._map1.tween(40000);
		this._map1.startRot(12000);
		this._map1.next(this._getMoji());
		haxe.Timer.delay($bind(this,this._tween),20000);
	}
	,_getMoji: function() {
...
var ModuleMap = function(type) {
	...
	if(type == 0) {
		...
		this._canvasName1 = "canvas3";
		this._canvasName2 = "canvas4";
	}
	this._type = type;
	this._canvas1 = window.document.getElementById(this._canvasName1);
	this._canvas2 = window.document.getElementById(this._canvasName2);
	this._div1 = new js.JQuery("#container" + this._type);
	this._div1.hide();
...
ModuleMap.prototype = {
	...
	,tween: function(time,callback) {
		if(time == null) time = 30000;
		console.log("tween");
		this._flag = false;
		this._div1.stop();
		this._div1.css({ top : data.StageSize.getHeight()});
		this._div1.animate({ top : -data.StageSize.getHeight()},time,"linear",callback);
		this._onResize(null);
	}
	,init: function(data) {
		this._loader = data;
	...
	,setMoji: function(moji) {
		...
		new js.JQuery("#region_no" + this._type).text("#" + this._data.id);
		new js.JQuery("#title" + this._type).text(this._data.title);
		new js.JQuery("#title" + this._type).off("click");
		new js.JQuery("#title" + this._type).on("click",$bind(this,this._goMap));
		new js.JQuery("#google" + this._type).off("click");
		new js.JQuery("#google" + this._type).on("click",$bind(this,this._goGoogle));
		new js.JQuery("#google" + this._type).text("earthview.withgoogle.com");
		new js.JQuery("#loading" + this._type).show();
		new js.JQuery("#loading" + this._type).text("LOADING");
		new js.JQuery("#title" + this._type).text(this._data.title);
		if(this._typo != null) this._stage2.removeChild(this._typo);
		this._typo = new MainDrawer();
		this._typo.init(this._data,moji,$bind(this,this._onLoadMainDrawer));
	...
	,_onLoadMainDrawer: function() {
		...
		this._bg.init(this._typo.getImage());
	...
	,_onResize: function(e) {
		this._canvas1.width = window.innerWidth;
		this._canvas1.height = data.StageSize.getHeight() + 2;
		this._canvas2.width = window.innerWidth;
		this._canvas2.height = data.StageSize.getHeight() + 2;
		if(this._stage1 != null) {
			this._stage1.clear();
			this._stage2.clear();
		}
